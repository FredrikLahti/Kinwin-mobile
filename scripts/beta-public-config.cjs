...
function validateBetaPublicConfig(env) {
  ...
  let invitation;
  try { invitation = new URL(env.EXPO_PUBLIC_RECIPIENT_INVITATION_BASE_URL ?? ''); } catch { throw new Error('Beta builds require a valid invitation HTTPS origin.'); }
  if (invitation.protocol !== 'https:' || invitation.pathname !== '/' || invitation.search || invitation.hash || invitation.username || invitation.password) throw new Error('Beta invitation configuration must be an HTTPS origin without path, token, query, or credentials.');
  return { invitationHost: invitation.host };
}
