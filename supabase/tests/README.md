...
| `240_server_scheduled_challenge_completion.sql` | Scheduled eligibility and reconciliation for elapsed active/Completion Mode challenges; exclusion of open, superseded, canceled, and pending rows; repeat safety; exactly-once success/failure activity; absence of payment/reward writes; service-only grants; worker lease and run-history observability. |
| `250_stripe_failure_charging.sql` | Only `completed_failure` produces one obligation; amount/currency are copied from trusted consequence rows; webhook-confirmed paid truth and duplicate-event safety; unchanged challenge truth; no reward fulfillment; and service-only claim authority. |
