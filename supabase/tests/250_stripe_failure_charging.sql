...
begin
 ...
 insert into public.challenges(
   ...
   c,o,1,1,p_status,'Europe/Stockholm',now()-interval '15 days',
   now()-interval '14 days',now()-interval '1 day',
   case when p_status in ('completed_success','completed_failure') then now() else null end,
   jsonb_build_object(
     'id',c::text,'ownerId',o::text,'schemaVersion',1,'ruleEngineVersion',1,
     'goal','Payment fixture',
...
select test.assert_equals('only_completed_failure_creates_obligation',(
  select count(*) from private.consequence_charge_attempts a
  join public.consequences co on co.id=a.consequence_id
  where co.challenge_id::text like 'a2000000-0000-0000-0000-%'
),1::bigint);
select test.assert_equals('amount_and_currency_are_locked_from_consequence',(
  select a.amount_minor_units||':'||a.currency from private.consequence_charge_attempts a
  join public.consequences co on co.id=a.consequence_id
...
select test.assert_equals('verified_success_marks_paid_once',(select status from private.consequence_charge_attempts where stripe_payment_intent_id='pi_payment_1'),'succeeded');
...
select test.assert_equals('no_reward_fulfillment_is_created',(
  select count(*) from private.reward_fulfillments r join public.consequences co on co.id=r.consequence_id
  where co.challenge_id='a2000000-0000-0000-0000-000000000001'
),0::bigint);
