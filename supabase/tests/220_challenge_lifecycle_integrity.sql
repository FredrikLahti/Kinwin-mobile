...
begin
  ...
  -- X: draft 2, also ready -- inserted directly as service_role so this
  -- file can test activate_challenge_draft's own guard independently of
  -- prepare_challenge_from_draft's (already tested separately below).
  insert into public.challenge_drafts (id, owner_id, schema_version, draft_payload, draft_status) values (
    'c2222222-0000-0000-0000-000000000002', 'c1111111-0000-0000-0000-000000000001', 1,
    jsonb_build_object(
      'schemaVersion', 1, 'id', 'c2222222-0000-0000-0000-000000000002', 'ownerId', 'c1111111-0000-0000-0000-000000000001',
      'goal', 'Read more', 'behavior', jsonb_build_object('description', 'Read before bed', 'completionDefinition', 'Read for 20 minutes', 'rule', jsonb_build_object('direction', 'build')),
  ...
  insert into public.challenges (id, owner_id, source_draft_id, schema_version, rule_engine_version, challenge_status)
    values ('c3333333-0000-0000-0000-000000000002', 'c1111111-0000-0000-0000-000000000001', 'c2222222-0000-0000-0000-000000000002', 1, 1, 'pending_activation');
  insert into public.consequences (id, challenge_id, owner_id, status, stake_minor_units, currency, authorization_status, authorized_at)
    values (gen_random_uuid(), 'c3333333-0000-0000-0000-000000000002', 'c1111111-0000-0000-0000-000000000001', 'authorized', 5000, 'USD', 'authorized', now());

  -- X: draft 3, a third pending row for the direct DB-level index test below.
  insert into public.challenges (id, owner_id, source_draft_id, schema_version, rule_engine_version, challenge_status)
    values ('c3333333-0000-0000-0000-000000000003', 'c1111111-0000-0000-0000-000000000001', null, 1, 1, 'pending_activation');
...
-- Activating the second pending challenge while the first is active is
-- rejected -- activate_challenge_draft's own new guard.
...
-- also rejected -- prepare_challenge_from_draft's new guard, tested via
-- a fresh draft with no challenge row yet (so this exercises that path,
-- not the "another pending exists" path already covered by
-- 120_one_pending_commitment_per_owner.sql).
do $$
begin
  insert into public.challenge_drafts (id, owner_id, schema_version, draft_payload, draft_status) values (
...
select test.assert_fails('db_level_invariant_rejects_a_second_active_row_directly',
  $stmt$update public.challenges set challenge_status = 'active', timezone = 'Europe/Stockholm', activated_at = now(), starts_at = now(), planned_ends_at = now() + interval '14 days',
    activation_snapshot = jsonb_build_object('id', 'c3333333-0000-0000-0000-000000000003', 'ownerId', 'c1111111-0000-0000-0000-000000000001', 'schemaVersion', 1, 'ruleEngineVersion', 1, 'goal', 'x', 'behavior', jsonb_build_object('description', 'x', 'completionDefinition', 'x'), 'duration', jsonb_build_object('unit', 'week', 'value', 2), 'successRule', jsonb_build_object('direction', 'build', 'ruleVersion', 1), 'recipients', jsonb_build_array(jsonb_build_object('id', 'r1', 'name', 'Anna')), 'rewardOrganizer', jsonb_build_object('type', 'recipient', 'recipientId', 'r1'), 'consequenceCategory', 'dinner', 'stake', jsonb_build_object('minorUnits', 5000, 'currency', 'USD'), 'sitOutAcknowledged', true, 'membershipStatusAtActivation', 'trialing')
    where id = 'c3333333-0000-0000-0000-000000000003'$stmt$,
  '23505');
reset role;
...
begin
  ...
  update public.consequences set authorization_status = 'authorized', authorized_at = now() where challenge_id = v_challenge_id;
end;
$$;
reset role;
set role service_role;
...
begin
  ...
  -- Simulate the post-repair state directly (this file's own activation
  -- flow above never leaves a second row 'active' to supersede for real,
  -- since the invariant now prevents that from ever happening again --
  -- this reproduces what the lifecycle migration's repair step did to
  -- Fredrik's historical rows).
