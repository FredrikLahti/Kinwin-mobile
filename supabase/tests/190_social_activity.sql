...
begin
  insert into auth.users (id, email) values
    ('91111111-0000-0000-0000-000000000001', 'social-d@example.test'),
    ('91111111-0000-0000-0000-000000000002', 'social-e@example.test'),
    ('91111111-0000-0000-0000-000000000003', 'social-f@example.test');

  insert into public.kin_connections (id, requester_id, recipient_id, status)
    values (gen_random_uuid(), '91111111-0000-0000-0000-000000000001', '91111111-0000-0000-0000-000000000002', 'accepted')
  ...
  insert into public.challenge_drafts (id, owner_id, schema_version, draft_payload, draft_status) values (
    '92222222-0000-0000-0000-000000000001', '91111111-0000-0000-0000-000000000001', 1,
    jsonb_build_object(
      'schemaVersion', 1, 'id', '92222222-0000-0000-0000-000000000001', 'ownerId', '91111111-0000-0000-0000-000000000001',
  ...
  insert into public.challenges (id, owner_id, source_draft_id, schema_version, rule_engine_version, challenge_status)
    values ('93333333-0000-0000-0000-000000000001', '91111111-0000-0000-0000-000000000001', '92222222-0000-0000-0000-000000000001', 1, 1, 'pending_activation');
  insert into public.consequences (id, challenge_id, owner_id, status, stake_minor_units, currency, authorization_status, authorized_at)
    values (gen_random_uuid(), '93333333-0000-0000-0000-000000000001', '91111111-0000-0000-0000-000000000001', 'authorized', 5000, 'USD', 'authorized', now());
  ...
  update public.challenges set
    ...
    activation_snapshot = jsonb_build_object(
      'id', '93333333-0000-0000-0000-000000000001', 'ownerId', '91111111-0000-0000-0000-000000000001',
...
select test.assert_equals('owner_sees_own_activity',
  (select count(*) from public.social_activity where owner_id = '91111111-0000-0000-0000-000000000001'), 1::bigint);
reset role;
...
set role authenticated;
select set_config('request.jwt.claim.sub', '91111111-0000-0000-0000-000000000003', false);
select test.assert_equals('stranger_cannot_see_activity',
  (select count(*) from public.social_activity where owner_id = '91111111-0000-0000-0000-000000000001'), 0::bigint);
reset role;
...
select test.assert_fails('authenticated_cannot_insert_social_activity_directly',
  $stmt$insert into public.social_activity (owner_id, challenge_id, kind, payload, dedupe_key)
    values ('91111111-0000-0000-0000-000000000001', '93333333-0000-0000-0000-000000000001', 'challenge_started', jsonb_build_object('x', 1), 'fake:1')$stmt$,
  '42501');
reset role;
...
declare
  ...
  v_affected bigint;
begin
  select id into v_activity_id from public.social_activity where owner_id = '91111111-0000-0000-0000-000000000001' and kind = 'challenge_started';
  insert into public.activity_reactions (activity_id, user_id, kind) values (v_activity_id, '91111111-0000-0000-0000-000000000002', 'respect');
...
select test.assert_fails('one_reaction_per_user_per_activity',
  $stmt$insert into public.activity_reactions (activity_id, user_id, kind)
    select id, '91111111-0000-0000-0000-000000000002', 'brutal' from public.social_activity
    where owner_id = '91111111-0000-0000-0000-000000000001' and kind = 'challenge_started'$stmt$,
  '23505');
-- Cannot react as someone else.
select test.assert_fails('cannot_react_as_another_user',
  $stmt$insert into public.activity_reactions (activity_id, user_id, kind)
    select id, '91111111-0000-0000-0000-000000000001', 'nice' from public.social_activity
    where owner_id = '91111111-0000-0000-0000-000000000001' and kind = 'challenge_started'$stmt$,
  '42501');
reset role;
...
select test.assert_fails('stranger_cannot_react_to_unrelated_activity',
  format($stmt$insert into public.activity_reactions (activity_id, user_id, kind) values (%L::uuid, '91111111-0000-0000-0000-000000000003', 'ouch')$stmt$,
    (select id from public.social_activity where owner_id = '91111111-0000-0000-0000-000000000001' and kind = 'challenge_started')),
  '42501');
select test.assert_equals('stranger_reads_zero_reactions',
  (select count(*) from public.activity_reactions where user_id = '91111111-0000-0000-0000-000000000002'), 0::bigint);
reset role;
...
declare
  v_affected bigint;
begin
  delete from public.activity_reactions where user_id = '91111111-0000-0000-0000-000000000002';
  get diagnostics v_affected = row_count;
...
select test.assert_fails('authenticated_cannot_call_finalize_challenge_result',
  $stmt$select public.finalize_challenge_result(
    '91111111-0000-0000-0000-000000000001', '93333333-0000-0000-0000-000000000001', 'completed_success',
