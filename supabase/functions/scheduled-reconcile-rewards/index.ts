...
import { createTremendousReconciliationAdapter, readTremendousReconciliationConfig } from '../_shared/tremendous/adapter.ts';
...
export default { fetch: withSupabase<any>({ auth: 'secret:default' }, async (req, ctx) => {
  ...
  const config = readTremendousReconciliationConfig((name) => Deno.env.get(name));
  if (!config) return Response.json({ error: 'reconciliation_contract_not_configured' }, { status: 503 });
  ...
  try {
    const result = await runRewardReconciliationWorker({
      ...
      claim: async (runId, token) => (await rpc('claim_due_reward_reconciliations', { p_run_id: runId, p_lease_token: token, p_limit: 25 }) as any[]).map((row) => ({ obligationId: row.obligation_id, providerRewardId: row.provider_reward_id })),
      retrieve: createTremendousReconciliationAdapter(config),
      record: async (runId, token, item, providerResult: TremendousReconciliationResult) => { await rpc('record_reward_reconciliation_result', { p_obligation_id: item.obligationId, p_run_id: runId, p_lease_token: token, p_result: providerResult.kind, p_provider_status: providerResult.kind === 'failure' ? providerResult.providerStatus : providerResult.providerStatus, p_redemption_url: providerResult.kind === 'ready' ? providerResult.redemptionUrl : null, p_retryable: providerResult.kind === 'failure' ? providerResult.retryable : false, p_failure_code: providerResult.kind === 'failure' ? providerResult.code : null }); },
