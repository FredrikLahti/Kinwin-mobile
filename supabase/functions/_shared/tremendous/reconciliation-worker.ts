...
export type ReconciliationItem = { readonly obligationId: string; readonly providerRewardId: string; readonly providerOrderId: string };
export type ReconciliationWorkerDependencies = {
  ...
  retrieve(providerRewardId: string, providerOrderId: string): Promise<TremendousReconciliationResult>;
...
export async function runRewardReconciliationWorker(dependencies: ReconciliationWorkerDependencies) {
  ...
  try {
    ...
    for (const row of rows) {
      const result = await dependencies.retrieve(row.providerRewardId, row.providerOrderId); attempted += 1;
