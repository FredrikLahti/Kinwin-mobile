...
test('worker records provider result once for each claimed obligation', async () => {
  ...
  const result = await runRewardFulfillmentWorker({ start: async () => ({ status: 'started', runId: 'r1', leaseToken: 't1' }), claim: async () => [item], create: async () => ({ ok: true, reward: { orderId: 'order', rewardId: 'reward' } }), record: async (_r, _t, row) => { recorded.push(row.idempotencyKey); }, finish: async () => {} });
