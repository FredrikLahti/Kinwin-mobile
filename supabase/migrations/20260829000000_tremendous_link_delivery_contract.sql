-- Official Tremendous LINK contract: delivery readiness comes from
-- GET /rewards/{id}; reward links are generated on demand and never stored.
...
where f.consequence_id=co.id and f.fulfillment_provider='tremendous_sandbox'
  and f.status='delivered' and f.provider_status is distinct from 'SUCCEEDED'
  and co.status='reward_delivered';
update private.reward_fulfillments set status='provider_created',delivered_at=null,
  reconciliation_next_retry_at=clock_timestamp()
where fulfillment_provider='tremendous_sandbox' and status='delivered'
  and provider_status is distinct from 'SUCCEEDED';

alter table private.reward_fulfillments drop column redemption_url;
...
alter table private.reward_fulfillments add constraint reward_fulfillments_delivered_evidence check(
  status<>'delivered' or (delivered_at is not null and provider_status='SUCCEEDED'
    and provider_order_id is not null and provider_reward_id is not null)
);

create or replace function private.protect_reward_fulfillment_binding()
...
drop function public.record_reward_fulfillment_result(uuid,uuid,uuid,boolean,boolean,text,text,text,text);
create function public.record_reward_fulfillment_result(p_obligation_id uuid,p_run_id uuid,p_lease_token uuid,p_succeeded boolean,p_retryable boolean,p_provider_order_id text default null,p_provider_reward_id text default null,p_failure_code text default null)
...
begin
  ...
  update private.reward_fulfillments set provider_reference=case when p_succeeded then p_provider_order_id else provider_reference end,
...
drop function public.claim_due_reward_reconciliations(uuid,uuid,integer);
create function public.claim_due_reward_reconciliations(p_run_id uuid,p_lease_token uuid,p_limit integer default 25)
returns table(obligation_id uuid,provider_reward_id text,provider_order_id text)
language plpgsql security definer set search_path='' as $$ begin
  ...
  ) select f.id,f.provider_reward_id,f.provider_order_id from claimed f;
end $$;

drop function public.record_reward_reconciliation_result(uuid,uuid,uuid,text,text,text,boolean,text);
create function public.record_reward_reconciliation_result(p_obligation_id uuid,p_run_id uuid,p_lease_token uuid,p_result text,p_provider_status text default null,p_retryable boolean default false,p_failure_code text default null)
returns void language plpgsql security definer set search_path='' as $$
declare f private.reward_fulfillments%rowtype;
begin
  ...
  if p_result='ready' and p_provider_status is distinct from 'SUCCEEDED' then raise exception 'ready requires succeeded LINK delivery evidence' using errcode='22023'; end if;
...
begin
  select jsonb_build_object('status',case when f.status='delivered' and f.provider_status='SUCCEEDED' then 'ready'
  ...
  into result from private.reward_fulfillments f join private.accepted_reward_organizer_targets target
...
declare result jsonb;
begin
  select jsonb_build_object('providerRewardId',f.provider_reward_id) into result
  from private.reward_fulfillments f join private.accepted_reward_organizer_targets target
    on target.consequence_id=f.consequence_id and target.organizer_id=f.organizer_id and target.invitation_id=p_invitation_id
  where f.status='delivered' and f.provider_status='SUCCEEDED' and f.provider_order_id is not null and f.provider_reward_id is not null;
  return result;
end $$;

revoke all on function public.claim_due_reward_reconciliations(uuid,uuid,integer) from public,anon,authenticated;
revoke all on function public.record_reward_fulfillment_result(uuid,uuid,uuid,boolean,boolean,text,text,text) from public,anon,authenticated;
revoke all on function public.record_reward_reconciliation_result(uuid,uuid,uuid,text,text,boolean,text) from public,anon,authenticated;
...
grant execute on function public.record_reward_fulfillment_result(uuid,uuid,uuid,boolean,boolean,text,text,text) to service_role;
grant execute on function public.record_reward_reconciliation_result(uuid,uuid,uuid,text,text,boolean,text) to service_role;
