-- Challenge lifecycle integrity: PR #30 worked around a real data
-- problem (Fredrik's hosted account had SIX challenges rows
-- simultaneously left at challenge_status = 'active') by giving Home and
-- Kin one shared, canonical "most recent active row" read. That is a
-- legitimate defensive read, but it should never have been necessary as
-- the PRIMARY integrity mechanism -- this migration fixes the actual
-- root cause so a second genuine active row can never be created again.
--
-- Root cause, confirmed by directly inspecting the real hosted rows
-- (read-only): neither prepare_challenge_from_draft
-- (20260805000000/20260808000000) nor activate_challenge_draft
-- (20260811000000) ever checked whether the owner already had another
-- challenge in challenge_status = 'active' (or 'completion_mode', the
-- other "currently accountable" state). 20260808000000 already enforces
-- "one pending_activation commitment per owner" -- this migration adds
-- the equivalent, and more important, invariant for the actual `active`
-- state itself, both as an early, friendly rejection in both RPCs and,
-- decisively, as a real PostgreSQL uniqueness constraint so two
-- concurrent/racing calls can never both win.
--
-- All six of Fredrik's real rows were inspected before writing this
-- migration: every one has zero check_in_events, all were activated on
-- the same calendar day (repeated manual onboarding-flow testing), and
-- none has reached the end of even its first reporting period. None
-- represents a genuinely resolvable success or failure -- there is no
-- truthful outcome to compute for any of them (evaluateChallenge would
-- report `periods_not_closed` for every one). The only truthful
-- classification is: superseded by a later activation before any real
-- history accrued. No other owner in the hosted project has more than
-- one row in {'active','completion_mode'}.

-- 1. A new, honest lifecycle state for exactly this situation: a
-- challenge that was genuinely activated but never became the user's
-- resolved commitment because a later activation replaced it first. Not
-- a success, not a failure, never charges a consequence or sends a
-- reward -- the schema had no state for this before now.
...
alter table public.challenges add constraint challenges_challenge_status_check
  check (challenge_status in (
    ...
    'completed_failure', 'canceled_before_activation', 'superseded'
  ));

-- 2. Data repair BEFORE the constraint below can be created -- a unique
-- index cannot be created against rows that already violate it. Self-
-- protecting: if any non-canonical active/completion_mode row for any
-- owner ever has real check-in history, this aborts the whole migration
-- instead of guessing at that row's fate. (Verified read-only beforehand
-- that no such row currently exists.)
do $$
declare
...
begin
  ...
  where c.challenge_status in ('active', 'completion_mode')
    and c.id <> (
      select c2.id from public.challenges c2
      where c2.owner_id = c.owner_id and c2.challenge_status in ('active', 'completion_mode')
      order by c2.activated_at desc
      limit 1
    )
    and exists (select 1 from public.check_in_events e where e.challenge_id = c.id);

  if v_unsafe_count > 0 then
    raise exception 'refusing automatic lifecycle repair: % non-canonical active row(s) have real check-in history and must be classified manually, not auto-superseded', v_unsafe_count;
  end if;
...
-- 3. The actual invariant: PostgreSQL, not application code, guarantees
-- at most one row per owner is ever in {'active','completion_mode'} —
-- mirrors challenges_owner_one_pending_idx's exact pattern
-- (20260808000000_one_pending_commitment_per_owner.sql) for the same
-- reason: two concurrent/racing calls can never both win.
create unique index challenges_owner_one_active_idx
  on public.challenges (owner_id)
  where challenge_status in ('active', 'completion_mode');

-- 4. prepare_challenge_from_draft: an early, friendly rejection so a
-- user is never even asked to set up payment for a challenge they can
-- never activate. Full function body replaces
-- 20260808000000_one_pending_commitment_per_owner.sql's version — only
-- one new check is added, immediately after the existing "one pending
-- commitment" check; every other line is unchanged.
...
begin
  ...
  -- New: a currently active (or completion-mode) challenge must be
  -- finished or resolved before another one can even be prepared — see
  -- this migration's own header for the full rationale.
  if exists (
    select 1 from public.challenges
    where owner_id = caller and challenge_status in ('active', 'completion_mode')
  ) then
...
-- 5. activate_challenge_draft: the decisive, atomic gate. Full function
-- body replaces 20260811000000_full_activation.sql's version — two
-- changes: a pre-check right after the idempotent-already-active early
-- return (friendly rejection for the common case), and the final UPDATE
-- wrapped to translate a lost race against
-- challenges_owner_one_active_idx into the same typed domain error
-- instead of a raw constraint failure. Everything else is unchanged,
-- including the payment-authorization gate, which still runs first and
-- unconditionally.
...
begin
  ...
  -- New: the decisive one-active-challenge check. A friendly early
  -- rejection here; the UPDATE at the end is still the real, atomic,
  -- race-proof gate via challenges_owner_one_active_idx.
  if exists (
    select 1 from public.challenges
    where owner_id = caller and challenge_status in ('active', 'completion_mode') and id <> target_id
  ) then
  ...
  begin
    ...
    where id = target_id;
  exception when unique_violation then
    -- Lost a genuine race: a concurrent call activated a different
    -- challenge for this same owner between the pre-check above and this
    -- UPDATE. challenges_owner_one_active_idx is what actually prevented
    -- two active rows from ever existing — this just gives the caller
    -- the same typed error the pre-check would have given a moment earlier.
    raise exception 'you already have a different active challenge; finish or resolve it before activating another' using errcode = '22023';
  end;
