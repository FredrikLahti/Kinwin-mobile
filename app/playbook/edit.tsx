...
import { PLAYBOOK_CATEGORY_DESCRIPTIONS, PLAYBOOK_CATEGORY_LABELS, PLAYBOOK_CATEGORY_PROMPTS } from '@/lib/playbook/category-copy';
...
export default function EditPlaybookEntry() {
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="light" /><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    {sourceChallengeId && <Text style={styles.sourceNote}>This entry will stay connected to the challenge you just completed.</Text>}
    {loading ? <ActivityIndicator color={theme.colors.rosewood} /> : <>
      <Text style={styles.label}>TYPE</Text>
      <View style={styles.choices}>{PLAYBOOK_CATEGORIES.map((value) => <Pressable key={value} accessibilityLabel={PLAYBOOK_CATEGORY_LABELS[value]} accessibilityRole="radio" accessibilityState={{ checked: category === value }} onPress={() => setCategory(value)} style={({ pressed }) => [styles.choice, category === value && styles.choiceSelected, pressed && styles.pressed]}><Text style={[styles.choiceText, category === value && styles.choiceTextSelected]}>{PLAYBOOK_CATEGORY_LABELS[value]}</Text></Pressable>)}</View>
      <Text accessibilityLiveRegion="polite" style={styles.categoryDescription}>{PLAYBOOK_CATEGORY_DESCRIPTIONS[category]}</Text>

      <Text style={styles.label}>YOUR NOTE</Text><TextInputV2 accessibilityLabel="Playbook entry" keyboardAppearance="dark" maxLength={280} multiline onChangeText={setContent} placeholder={PLAYBOOK_CATEGORY_PROMPTS[category]} placeholderTextColor={theme.colors.warmGrey} returnKeyType="done" style={styles.input} textAlignVertical="top" value={content} />
      <View style={styles.inputMeta}>
        <Text style={styles.counter}>{content.length}/280</Text>
        {keyboardVisible && <Pressable accessibilityHint="Closes the keyboard" accessibilityRole="button" onPress={() => Keyboard.dismiss()} style={styles.doneButton}><Text style={styles.done}>Done</Text></Pressable>}
      </View>
