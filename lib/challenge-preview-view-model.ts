...
export type ActiveChallengeViewModel = {
  ...
  nextAction: { type: 'check_in' | 'update_total' | 'report_status' | 'continue_recovery' | 'review_playbook' | 'period_complete'; label: string; detail: string };
...
export function buildActiveChallengeViewModel(input: {
  ...
  return {
    ...
    twoFuturesState,
    consequenceState: 'preview_only_no_action',
    nextAction: deriveNextAction(configuration, currentPeriod, activeRecovery, latestInsight.kind === 'known'),
    latestInsight,
...
function deriveCurrentPeriod(configuration: PreviewConfiguration, events: readonly PreviewEvent[]): CurrentPeriodView {
  if (configuration.direction === 'build') {
    ...
    if (count === 0) return { status: 'not_recorded', recorded: { kind: 'known', value: 0 }, remaining: { kind: 'known', value: configuration.target }, headline: `0 of ${configuration.target} completed this period`, detail: `${configuration.target} more keep this period on track.` };
    const remaining = Math.max(configuration.target - count, 0);
    return { status: remaining === 0 ? 'met' : 'in_progress', recorded: { kind: 'known', value: count }, remaining: { kind: 'known', value: remaining }, headline: `${count} of ${configuration.target} completed this period`, detail: remaining === 0 ? 'The current period target is complete.' : `${remaining} more keep this period on track.` };
  }
  if (configuration.direction === 'cut') {
    ...
    if (!totals.length) return { status: 'not_recorded', recorded: { kind: 'not_recorded' }, remaining: { kind: 'not_recorded' }, headline: 'No total recorded for this period', detail: 'Update the current total when you have something to record.' };
    const value = totals.reduce((latest, event) => event.order > latest.order ? event : latest).value;
    ...
    if (difference < 0) return { status: 'over_limit', recorded: { kind: 'known', value }, remaining: { kind: 'known', value: difference }, headline: `${formatted} of ${limit} used`, detail: `${formatValue(Math.abs(difference), configuration.unit)} over the current limit.` };
    return { status: difference === 0 ? 'at_limit' : 'within_limit', recorded: { kind: 'known', value }, remaining: { kind: 'known', value: difference }, headline: `${formatted} of ${limit} used`, detail: difference === 0 ? 'The current limit has been reached.' : `${formatValue(difference, configuration.unit)} remain this period.` };
  }
  const statuses = events.filter((event): event is Extract<PreviewEvent, { type: 'stop_status' }> => event.type === 'stop_status');
  const lapse = statuses.some((event) => event.status === 'lapse');
  if (lapse) return { status: 'lapse', recorded: { kind: 'known', value: 'lapse' }, remaining: { kind: 'not_applicable' }, headline: 'A lapse has been recorded', detail: 'Stop V1 remains a zero-lapse rule. Recovery does not erase this event.' };
  if (statuses.some((event) => event.status === 'intact')) return { status: 'intact', recorded: { kind: 'known', value: 'intact' }, remaining: { kind: 'not_applicable' }, headline: 'Promise intact in this preview', detail: 'No lapse has been recorded in this preview session.' };
  return { status: 'not_recorded', recorded: { kind: 'not_recorded' }, remaining: { kind: 'not_applicable' }, headline: 'No status recorded yet', detail: 'Report whether the promise is still intact.' };
}

function deriveNextAction(configuration: PreviewConfiguration, period: CurrentPeriodView, activeRecovery: boolean, hasInsight: boolean): ActiveChallengeViewModel['nextAction'] {
  if (activeRecovery) return { type: 'continue_recovery', label: 'Continue recovery', detail: 'Complete the recovery action you chose.' };
  if (period.status === 'over_limit' || period.status === 'lapse') return { type: 'continue_recovery', label: 'Plan recovery', detail: 'Capture what made this harder and choose the next useful action.' };
