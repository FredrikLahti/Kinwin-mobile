...
export async function generateOrganizerRewardLink(token:string):Promise<Result<string>>{const config=readSupabaseConfig();if(!config)return failed('This reward is unavailable in this build.');const response=await fetch(`${config.url}/functions/v1/organizer-reward-link`,{method:'POST',headers:{apikey:config.anonKey,Authorization:`Bearer ${config.anonKey}`,'Content-Type':'application/json'},body:JSON.stringify({token})});if(!response.ok)return failed(response.status===404?'This reward is not available for this invitation.':'Could not open this reward.');const data=await response.json();return typeof data?.url==='string'?{ok:true,value:data.url}:failed('Could not open this reward.');}
