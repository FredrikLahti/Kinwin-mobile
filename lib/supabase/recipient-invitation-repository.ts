...
export type RecipientProjection = { readonly status: OwnerInvitationStatus; readonly ownerName: string; readonly recipientName: string|null; readonly goal: string; readonly behavior: string; readonly consequenceCategory: string; readonly ownerSitsOut: true; readonly accessRole:'recipient'|'organizer'; readonly organizerName:string|null; readonly recipientNames:readonly string[]; readonly rewardStatus:'preparing'|'processing'|'ready'|'needs_attention'|null };
...
export type RewardLinkResult={readonly ok:true;readonly value:string}|{readonly ok:false;readonly kind:'unavailable'|'cooldown'|'temporary'};
export async function generateOrganizerRewardLink(token:string):Promise<RewardLinkResult>{const config=readSupabaseConfig();if(!config)return{ok:false,kind:'temporary'};let response:Response;try{response=await fetch(`${config.url}/functions/v1/organizer-reward-link`,{method:'POST',headers:{apikey:config.anonKey,Authorization:`Bearer ${config.anonKey}`,'Content-Type':'application/json'},body:JSON.stringify({token})});}catch{return{ok:false,kind:'temporary'};}if(!response.ok)return{ok:false,kind:response.status===404?'unavailable':response.status===429?'cooldown':'temporary'};const data=await response.json();return typeof data?.url==='string'?{ok:true,value:data.url}:{ok:false,kind:'temporary'};}
