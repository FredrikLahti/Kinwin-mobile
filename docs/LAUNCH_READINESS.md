...
| Fixture-driven "Progress" surface (`app/home/progress.tsx`) | **EXTERNAL BETA BLOCKER.** Real, navigable screen in the shipping Home tabs, but every number on it (streak, consistency %, weekly bars) is static fixture data, not the tester's real data — easily mistaken for a genuine feature by an outside tester who doesn't know the codebase. Must be either (a) connected to real user data, or (b) removed/hidden from shipping navigation before outside beta testers receive the app. Not an internal first-install blocker, and does not require a full real-progress/statistics implementation — hiding the surface is an acceptable beta-scoped fix. See `docs/PRODUCT_STATUS.md` §5. |
