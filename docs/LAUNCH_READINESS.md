...
| Password reset | **CODE RESOLVED, HOSTED CONFIGURATION UNVERIFIED.** Real Supabase `resetPasswordForEmail`/`setSession`/`updateUser` flow shipped across `app/auth/forgot-password.tsx` and `app/auth/reset-password.tsx`, enumeration-safe throughout, using the implicit auth flow deliberately (see `lib/supabase/client.ts`). What could not be verified from this sandboxed environment (no hosted-project credentials or Supabase CLI/Management API access): the hosted TEST Supabase project's Auth → Redirect URLs allowlist must include this app's reset-password URL, or GoTrue will silently ignore `redirectTo` and the emailed link won't return to the app correctly. **Founder/operator action:** open the hosted TEST Supabase project's Auth settings and add the reset-password URL (the same `https://` origin `EXPO_PUBLIC_RECIPIENT_INVITATION_BASE_URL` already points at, e.g. `https://kinwin-beta.expo.app/auth/reset-password`) to the Redirect URLs allowlist, then send one real test reset email to confirm the round trip end to end. See `docs/PRODUCT_STATUS.md` §1. |
...
| Item | Status |
|---|---|
| Privacy policy | **PUBLIC LAUNCH BLOCKER, narrowed.** A factual DRAFT now exists (`app/legal/privacy.tsx`, publicly reachable without sign-in, linked from Account settings), built strictly from `docs/PRIVACY_DATA_INVENTORY.md`'s evidence-based data inventory. It is explicitly labeled as a draft and has **not** had legal review — still required before this can satisfy Apple's real, reachable privacy-policy requirement. |
| Terms of Service | **PUBLIC LAUNCH BLOCKER, unchanged.** Only route infrastructure exists (`app/legal/terms.tsx`), deliberately **not linked** from any shipping navigation and explicitly labeled "not ready" on the page itself, since no real Terms exist yet and showing an unfinished page under that name would be misleading. See `docs/PRODUCT_DECISIONS.md`'s consequence-consent gap analysis for what the current in-app disclosure already covers versus what a real Terms document still needs. |
| **Account deletion** | **PUBLIC LAUNCH BLOCKER, unchanged — flagging clearly per App Store Review Guideline 5.1.1(v): if account creation exists in the app (it does), Apple requires an in-app account-deletion path.** Still confirmed absent everywhere — no UI, no Supabase function, no migration, and **still not implemented in this package, by design.** A full decision package now exists — `docs/ACCOUNT_DELETION_DECISIONS.md` — covering active-challenge, pending-payment, reward-fulfillment, completed-history, Kin-relationship, invitation, and payment-record questions with recommended options, so the founder has a concrete package to decide against instead of open questions. See [Product decisions before public launch](#product-decisions-before-public-launch) for the short list of what still needs a founder answer. |
...
1. **Account deletion semantics.** Full decision package with technical constraints, trade-offs, and recommended options for each question now lives in `docs/ACCOUNT_DELETION_DECISIONS.md`. The five decisions that specifically need a founder answer (restated compactly from that document's own closing section):
   - Completed-history model: hard-delete vs. anonymize-in-place for the user's own data, and separately for activity/reactions still visible in another Kin's feed.
   - Whether to override the database's existing `on delete cascade` behavior for Kin connections with an explicit "removed" transition instead (follows from the previous decision).
   - How long to retain minimal Stripe/Tremendous provider-reference data after deletion, and in what form — needs legal/accounting advice, not an engineering guess.
   - Whether "block deletion during any non-terminal challenge/payment/reward state" is the final policy, or a different tradeoff is preferred despite the product-integrity concerns that recommendation is based on.
   - Whether recipients' typed display names on a deleted owner's challenges are retained as historical record or removed.
...
2. **Remaining external-beta surface.** Password reset, the support/contact surface, and export-compliance declaration are now shipped. Two founder/operator-side steps remain, neither engineering: add the reset-password URL to the hosted TEST Supabase project's Auth Redirect URLs allowlist (see the Password reset row above), and set `EXPO_PUBLIC_SUPPORT_EMAIL` to a real address. Also still open: a real legal review of the existing consequence-consent copy (see `docs/PRODUCT_DECISIONS.md`'s gap analysis for exactly what needs review).
3. **Public-launch package, after the product decisions above are answered.** Implementing account deletion (the decision package exists in `docs/ACCOUNT_DELETION_DECISIONS.md`; implementation is still gated on the founder answering the five decisions listed above), App Store Connect setup (metadata, screenshots, privacy nutrition labels using the working map in `docs/PRIVACY_DATA_INVENTORY.md`, production bundle identifier), legally reviewing the privacy policy draft and writing real Terms of Service (route infrastructure for both already exists — `app/legal/`), and the production-environment provisioning decision (Supabase/Stripe/Tremendous) — each still gated on an explicit founder decision, not an engineering guess.
