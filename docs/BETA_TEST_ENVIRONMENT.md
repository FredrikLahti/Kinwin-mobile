...
Setting these values (via `eas env:set` or a step's own shell `env:`) is necessary but not sufficient for them to reach the deployed web build. A 2026-08-15 regression identified why: `eas-beta-release.yml` runs `npx expo export --platform web` twice in the same job — once to claim the EAS Hosting origin with no config, once for real with all five values — and Metro's on-disk transform cache does not key on live `EXPO_PUBLIC_*` values, only file content, so without a cache reset the second export silently reused the first export's config-less cached bundle. Two permanent safeguards now guard against this: the workflow passes `--clear` on the real export so it can never reuse another export's cache, and a pre-deploy step verifies the exported bundle actually contains the Supabase project URL, failing the deploy before it ships if not. Do not remove either safeguard. HTTP-200 origin checks (below) remain useful but are not sufficient on their own — they only prove the server answers, not that the served build works — so treat each release that touches the web export as needing a functional spot-check (Sign in, an invitation link) after deploying.
