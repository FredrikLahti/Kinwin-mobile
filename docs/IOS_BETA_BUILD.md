...
cannot be fabricated in the repository, but the file itself is generated deterministically:
`npm run build:web:beta` runs `expo export --platform web --output-dir dist-beta-web` (Expo Router
web output `single`) and then writes `dist-beta-web/.well-known/apple-app-site-association` from
`scripts/generate-beta-aasa.cjs`, which requires a real, non-placeholder `KINWIN_APPLE_TEAM_ID` and
...
- A real `KINWIN_APPLE_TEAM_ID` and a public beta web host that deploys `npm run build:web:beta`'s
  output (including the generated AASA file) behind the `/invite/*` rewrite
- Supabase TEST Site URL and hosted backend deployment

These are release credentials or hosting decisions, not values Codex should invent or commit.
