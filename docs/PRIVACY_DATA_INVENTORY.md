...
| Contact Info → Name | Yes (display name, optional) | Yes | App Functionality (personalization) | No |
| Contact Info → Phone Number, Physical Address | No | — | — | No |
| Contact Info → Other User Contact Info | See note below | — | — | No |
| Financial Info → Payment Info | See note below | — | — | No |
| Financial Info → Other Financial Info | Yes (stake amount, currency) | Yes | App Functionality | No |
| Identifiers → User ID | Yes (Supabase user id) | Yes | App Functionality | No |
| Identifiers → Device ID | No | — | — | No |
| User Content → Other User Content | Yes (goals, behaviors, check-ins, Playbook entries) | Yes | App Functionality | No |
| Usage Data (Product Interaction, Advertising Data) | No — no analytics SDK exists | — | — | No |
...
**Two items marked "see note" — genuine open questions, not resolved here:**
1. **"Other User Contact Info"** (recipient/organizer display names): these are free-text names of *other people* the app's user enters, not the app user's own contact info. Apple's questionnaire is written from the perspective of data collected "from your app about a particular end-user" — whether a recipient's typed name triggers this category the same way importing a device contact list would is a real interpretive question. Needs to be checked against Apple's exact current questionnaire wording at submission time, not assumed either way here.
2. **"Payment Info"**: Kinwin's own servers never receive raw card data (Stripe's PaymentSheet SDK collects it directly and it never transits Kinwin's own code or database — see §4 above). Apple's current App Privacy Details guidance has specific, narrower treatment for apps using a third-party payment processor SDK where the developer never receives the payment data, but the exact current wording and whether it fully exempts this from declaration needs to be checked directly in the App Store Connect questionnaire at submission time — not assumed from this document.
