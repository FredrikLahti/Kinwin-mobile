...
| Contact Info → Other User Contact Info | No. This category is for an end-user-supplied external contact channel (another person's email, phone number, or physical address); Kinwin never collects any of those for a recipient or organizer, only a typed display name, which belongs under Contact Info → Name instead (see above and the note below) | — | — | No |
