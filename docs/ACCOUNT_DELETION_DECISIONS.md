...
**This document does not implement account deletion.** It exists so the founder can make the decisions engineering should not make alone, before any deletion code is written. Apple requires an in-app path to *initiate* account deletion for any app with account creation (App Store Review Guideline 5.1.1(v)) — Kinwin has account creation, so this is a real public-launch requirement, not a nice-to-have. It is **not** required for internal, ad-hoc founder/close-circle testing. It is **not automatically exempt for external TestFlight distribution**, however: Apple's Guideline 2.2 treats TestFlight-distributed builds as subject to App Review compliance, so if the distribution-model decision (see `docs/LAUNCH_READINESS.md`) lands on TestFlight rather than continued ad-hoc distribution, account deletion may become a requirement at that step, not only at final public App Store submission. This is a real, undecided dependency on which distribution model gets chosen, not a settled exemption.
...
**Implementation consequence:** invitation rows are included in the ordered delete path for the `challenges` chain, not treated as an exception to it. If any audit trace is required, it is written to the separate minimal-record structure described in Payment/provider records below *before* that delete happens — never assumed to survive by simply not deleting the original row.
...
**Recommendation: Option 1**, scoped as narrowly as possible. **This is exactly the kind of retention period and record design that needs real legal/accounting advice, not an engineering guess** — no specific number of days/years is proposed here, and the "separate retained-record structure" is conceptual only: no such table exists yet, and it is explicitly not designed or built in this document.

**Implementation consequence:** the deletion flow should be designed around "copy the minimal transaction record out, then delete the person and the rest of the challenge/consequence graph" rather than either "delete everything" or "leave the restrict-chained rows in place" (the latter is not achievable under the current schema once `challenges` is actually deleted). The exact boundary, field list, and retention length remain a founder/legal decision, not resolved by this document — but the *shape* of the solution (a decoupled record, built before the delete, not a surviving live row) is a real constraint, not a choice.
...
1. **Completed-history model**: hard-delete vs. anonymize-in-place for a user's own challenge/check-in/Playbook/`social_activity`/`activity_reactions` data, including the slice still visible in another Kin's feed (recommendation: hard-delete across the board, matching Apple's current guidance that users expect UGC shared with others to be deleted, not anonymized; anonymization remains available as a stated-cost option if the founder prefers it).
2. **Kin relationship handling**: whether the existing DB-level `on delete cascade` behavior for `kin_connections` is acceptable as-is (recommendation: yes, consistent with decision #1), or whether the founder wants a real tombstone/archival model instead — noting that is a genuine, currently-undesigned schema project, not a status-column change.
3. **Minimal retained-record design and retention period**: whether to build the proposed decoupled, private minimal-record structure for Stripe/Tremendous provider references and any necessary invitation-acceptance audit facts, what exact fields it holds, and how long Kinwin should keep it after account deletion (recommendation: build something narrow along these lines, but the exact design and period need real legal/accounting advice, not proposed here).
...
5. **Whether recipients' typed display names are retained anywhere after a deleted owner's challenge is removed** — under the recommendation in #3, the answer should default to no (a de-identified audit fact, not a name), but this is called out separately since recipients never had a Kinwin account of their own to consent to any retention.
