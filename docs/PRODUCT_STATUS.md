...
| Privacy policy surface | PARTIAL | `app/legal/privacy.tsx` is a real, publicly reachable (no sign-in required) factual DRAFT built strictly from `docs/PRIVACY_DATA_INVENTORY.md`, linked from Account settings; explicitly labeled as a draft, not legally reviewed. A functional (not legally reviewed) consequence-disclosure also exists in the creation flow — see §2 and `docs/PRODUCT_DECISIONS.md`'s gap analysis. | Legal review of the Privacy draft remains outstanding, and is a conditional external-TestFlight blocker as well as a public-launch one — see `LAUNCH_READINESS.md`. |
| Custom Kinwin Terms of Service | NOT IMPLEMENTED — legal/business decision, not a generic Apple requirement | Apple applies its own standard EULA to App Store Connect submissions by default when no custom EULA is supplied, so a Kinwin-specific Terms document is not universally required just to publish an app. The previous unlinked internal-placeholder route (`app/legal/terms.tsx`) has been removed from the shipping app in this pass — Apple's App Review guidance discourages placeholder/incomplete content, and there was no product need to ship it. | Whether Kinwin's consequence/payment/reward model needs its own Terms before real-money public launch is an open legal/business question, not resolved here. |
...
| Support/contact surface | See §1 | Founder-facing support/contact channel for testers — tracked once in §1 as PARTIAL (real UI, needs a real address configured). Not duplicated here. | See `LAUNCH_READINESS.md` external-beta gate. |
...
- Legal review of the Privacy draft (`app/legal/privacy.tsx`). Also a conditional external-TestFlight blocker, not only public-launch, if TestFlight is the chosen distribution model.
