...
| Account deletion | IMPLEMENTED | Real, server-backed deletion: `app/account/delete-account.tsx` (low-prominence entry from Account, server-checked eligibility preflight, explicit destructive confirmation, real completion state), `supabase/functions/delete-account`, `supabase/migrations/20260903000000_account_deletion.sql`. Blocked outright while any challenge/payment/reward obligation is non-terminal; once terminal, hard-deletes the account's own challenge history, check-ins, Playbook entries, social activity, Kin connections, and (TEST-only) provider references, then `auth.users` via the Admin API — see `docs/ACCOUNT_DELETION_DECISIONS.md` for the founder decisions this implements. Tested at `supabase/tests/340_account_deletion.sql` and `supabase/tests/e2e/account-deletion.e2e.ts`. | Production, real-money retention design for provider references is explicitly deferred — see `docs/ACCOUNT_DELETION_DECISIONS.md`'s "Payment / provider records" section. |
...
### Public-launch work
- ~~Account deletion implementation~~ **IMPLEMENTED** — see §2 above and `docs/ACCOUNT_DELETION_DECISIONS.md`. One deferred piece remains for real-money launch: the minimal retained-record design for production Stripe/Tremendous provider references, which needs real legal/accounting advice.
- Finishing the Privacy policy into a publication-ready document satisfying Apple Guideline 5.1.1 (`app/legal/privacy.tsx` currently self-labels as a factual DRAFT — its retention/deletion story is now factually complete, since account deletion is real). Also a conditional external-TestFlight blocker, not only public-launch, if TestFlight is the chosen distribution model. A separate legal review of the final wording is recommended before public real-money launch, but is not itself the Apple requirement.
...
1. ~~Account deletion semantics~~ — **decided and implemented**, see `docs/ACCOUNT_DELETION_DECISIONS.md`'s "Founder decisions — as made, and what is implemented" section. Only the production, real-money minimal-record retention design for Stripe/Tremendous provider references remains open, and needs real legal/accounting advice.
