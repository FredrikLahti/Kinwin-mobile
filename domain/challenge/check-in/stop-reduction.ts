...
export type StopHistoryResult =
  | {
      ...
      /** True if an effective `stop_intact` entry exists timestamped in [period.endsAt, period.reportingClosesAt) — the window the final result checks. */
...
export function resolveStopHistory(period: ChallengePeriod, eventsForPeriod: readonly CheckInEvent[]): StopHistoryResult {
  ...
  const hasFinalIntactAttestation = chainEntries.some(
    (entry) =>
      ...
      compareIso(entry.terminalRecordedAt, period.endsAt) >= 0 &&
      compareIso(entry.terminalRecordedAt, period.reportingClosesAt) < 0,
  );
